...
- **`GatewayDirectAgent`** (`src/process/task/directAgent/`) é o agente de resposta dos canais: um agente leve, sem CLI, que fala `/v1/chat/completions` compatível com OpenAI ao gateway com um token Firebase novo. É uma porta comportamento-por-comportamento do `GeminiDirectAgent` nativo (BYOK) — mesma persona de recepcionista, streaming e function-calling do Google Calendar (que funciona pelo protocolo de tools da OpenAI). A fábrica de agentes o escolhe sempre que o modelo da conversa é `isGateway`.
...
- **Modelo sempre atual** — uma conversa de canal do tipo `gemini` re-resolve seu modelo a partir do padrão salvo do canal a cada mensagem, então uma troca de modelo em Configurações (ou no cabeçalho do chat) passa a valer imediatamente e conversas legadas migram de qualquer modelo obsoleto/BYOK.
...
- Canais: respostas de modo-negócio de WhatsApp + Telegram roteadas pelo gateway via `GatewayDirectAgent` (compatível com OpenAI, token novo), seleção de modelo exclusiva do gateway com padrão de baixo custo e resolução sempre-atual a cada mensagem. Verificado de ponta a ponta, incluindo chamadas de tool do Google Calendar.
